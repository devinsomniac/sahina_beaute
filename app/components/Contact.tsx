...
const contactItems = [
  ...
  },
  {
    icon: "🚌",
    label: "Transport",
    value: ["Bus 170 · T1 · T8 · RER D & H", "Saint-Denis Gare"],
  },
];
  ...
  return (
    <section id="contact" className="grid grid-cols-1 lg:grid-cols-2">
      <div className="bg-[#f6e9eb] px-6 py-16 md:px-10 lg:px-16 xl:px-20">
        ...
        >
          Visit <span className="italic text-[#C9A84C]">Sahina Beauté</span>
        </h2>
        ...
        <div className="mt-10 flex flex-col gap-7">
          {contactItems.map((item) => (
            <div key={item.label} className="flex gap-4">
              ...
              <div>
                <p className={`${nunito.className} uppercase tracking-[0.25em] text-[11px] text-[#C9A84C]`}>
                  {item.label}
                </p>
                <div className={`${nunito.className} mt-1 space-y-1 text-[#9A6070]`}>
                  {item.value.map((line) => (
                    <p key={line}>{line}</p>
                  ))}
                </div>
        ...
        <div className="mt-10">
          <a
            ...
            rel="noreferrer"
          >
            <Button className={`${nunito.className} rounded-full bg-[#7c0828] px-8 uppercase tracking-[0.18em] text-xs text-white hover:bg-[#8f1236]`}>
              📍 Open in Google Maps
            </Button>
          </a>
        </div>
      ...
      <div className="bg-[#fcf9fa] px-6 py-16 md:px-10 lg:px-16 xl:px-20">
        <div className={`${nunito.className} inline-flex items-center gap-2 rounded-full border border-[#e3cfd5] bg-[#f7eef1] px-4 py-2 uppercase tracking-[0.2em] text-xs text-[#9A6070]`}>
          🕘 Open 7 days a week
        </div>
        ...
        <p className={`${nunito.className} mt-8 max-w-2xl leading-8 text-[#9A6070]`}>
          Walk-ins are always welcome. For bridal packages or larger group
          appointments, calling ahead is recommended to ensure availability.
        </p>

        <div className="mt-10">
          <a href="#booking">
            <Button className={`${nunito.className} h-12 w-full rounded-full bg-[#7c0828] uppercase tracking-[0.18em] text-xs text-white shadow-lg hover:bg-[#8f1236]`}>
              Book Appointment →
            </Button>
          </a>
        </div>
